/**
 * Products L2 — Custom Modules series page data
 * 3 SKUs: CN180W / CN200W / CN215W
 */

export interface CustomSku {
  key: string;
  title: string;
  subtitle: string;
  power: string;
  efficiency: string;
  description: string;
  specs: Array<{ label: string; value: string }>;
  image: string;
  detailHref: string;
  datasheetHref?: string;
}

export const CUSTOM_L2 = {
  breadcrumb: [
    { label: 'PRODUCTS', href: '/products/' },
    { label: 'CUSTOM PV MODULES', href: '/products/custom-modules/' },
  ],
  hero: {
    title: 'Custom PV Modules',
    subtitle:
      'Application-specific modules for RVs, vehicle rooftops, balconies, off-grid equipment, and other special installation conditions.',
    powerRange: '180W – 215W',
    powerRange: '180W – 215W',
    tags: ['SIZE ADAPTATION', 'STRUCTURAL ENGINEERING', 'CONNECTION COMPATIBILITY', '24V SYSTEM READY'],
    cta: { label: 'DISCUSS A PROJECT', href: '#inquiry' },
    thumbs: [
      '/uploads/callsun/products/210w/card-logo.webp',
      '/uploads/callsun/products/180w/card-logo.webp',
      '/uploads/callsun/products/200w/card-logo.webp',
      '/uploads/callsun/products/215w/card-logo.webp',
    ],
  },
  selectorEyebrow: 'SELECT A MODULE',
  overview: {
    eyebrow: 'SERIES OVERVIEW',
    title: 'Engineered Around Available Space',
    intro:
      'Module selection and custom feasibility assessment can consider available space, target output, module dimensions, product structure and interface requirements.',
  },
  skus: [
    {
      key: 'cn180w',
      title: 'CN180W',
      subtitle: 'CN180W Custom N-Type Bifacial Module',
      power: '180W',
      efficiency: '25.6% CELL EFFICIENCY',
      description:
        'Anti-shading TwinCell design keeps output stable under partial shade from trees, roof racks and uneven sunlight — a good fit for RV rooftops, vans and vehicle-mounted installations.',
      specs: [
        { label: 'DIMENSIONS', value: '1180 × 770 × 35 mm' },
        { label: 'WEIGHT', value: '9.5 kg' },
        { label: 'OPERATING POINT', value: 'Vmp 22.35 V · Imp 8.05 A' },
        { label: 'OPEN / SHORT CIRCUIT', value: 'Voc 25.82 V · Isc 8.86 A' },
        { label: 'DESIGN', value: 'Compact Footprint · Anti Shade Design · Bifacial Energy Gain' },
      ],
      image: '/uploads/callsun/products/180w/card-logo.webp',
      detailHref: '/products/cn180w/',
      datasheetHref: '/uploads/callsun/datasheets/CN180W_Solar_Module_Datasheet.pdf',
    },
    {
      key: 'cn200w',
      title: 'CN200W',
      subtitle: 'CN200W Custom N-Type Bifacial Module',
      power: '200W',
      efficiency: '25.6% CELL EFFICIENCY',
      description:
        'Ultra-compact 200W with higher watt density per square metre for balcony PV, small off-grid gear and space-constrained rooftop setups where every watt per square metre counts.',
      specs: [
        { label: 'DIMENSIONS', value: '1304 × 770 × 35 mm' },
        { label: 'WEIGHT', value: '10.5 kg' },
        { label: 'OPERATING POINT', value: 'Vmp 24.83 V · Imp 8.05 A' },
        { label: 'OPEN / SHORT CIRCUIT', value: 'Voc 28.68 V · Isc 8.86 A' },
        { label: 'DESIGN', value: 'High Temperature Performance · Bifacial Energy Gain · Long Term Power Output' },
      ],
      image: '/uploads/callsun/products/200w/card-logo.webp',
      detailHref: '/products/cn200w/',
      datasheetHref: '/uploads/callsun/datasheets/CN200W_Solar_Module_Datasheet.pdf',
    },
    {
      key: 'cn210w',
      title: 'CN210W',
      subtitle: 'CN210W Custom N-Type Bifacial Module',
      power: '210W',
      efficiency: '25.6% CELL EFFICIENCY',
      description:
        'Dual-module parallel circuitry with 1/4-cut cells tuned for 24V systems — purpose-built for RV, marine, off-grid cabins and solar generator applications where partial shade is common and reliable 24V output is critical.',
      specs: [
        { label: 'DIMENSIONS', value: '1376 × 766 × 35 mm' },
        { label: 'WEIGHT', value: '11.3 kg' },
        { label: 'OPERATING POINT', value: 'Vmp 34.2 V · Imp 6.1 A' },
        { label: 'OPEN / SHORT CIRCUIT', value: 'Voc 40.3 V · Isc 6.5 A' },
        { label: 'DESIGN', value: 'High Voltage Output · 12V and 24V Compatibility · Anti Shade Design' },
      ],
      image: '/uploads/callsun/products/210w/card-logo.webp',
      detailHref: '/products/cn210w/',
      datasheetHref: '/uploads/callsun/datasheets/CN210W_Solar_Module_Datasheet.pdf',
    },
    {
      key: 'cn215w',
      title: 'CN215W',
      subtitle: 'CN215W Custom N-Type Bifacial Module with Anti-Shading',
      power: '215W',
      efficiency: '25.6% CELL EFFICIENCY',
      description:
        '4-zone anti-shading design tuned for 12 V systems and off-grid equipment where partial shade would otherwise hit output hard — well suited to vehicle and marine PV.',
      specs: [
        { label: 'DIMENSIONS', value: '1422 × 766 × 35 mm' },
        { label: 'WEIGHT', value: '11.5 kg' },
        { label: 'OPERATING POINT', value: 'Vmp 17.30 V · Imp 12.43 A' },
        { label: 'OPEN / SHORT CIRCUIT', value: 'Voc 20.09 V · Isc 12.98 A' },
        { label: 'DESIGN', value: '4 Module Anti Shade Design · 12V System Compatibility · Bifacial Energy Gain' },
      ],
      image: '/uploads/callsun/products/215w/card-logo.webp',
      detailHref: '/products/cn215w/',
      datasheetHref: '/uploads/callsun/datasheets/CN215W_Solar_Module_Datasheet.pdf',
    },
  ] as CustomSku[],
  inquiry: {
    eyebrow: 'PRODUCT INQUIRY',
    title: 'Discuss Your Module Requirements',
    intro:
      'Share the target power, module dimensions, application, installation conditions and expected volume. We will review the requirements and respond with the relevant product or next step.',
    formTitle: 'Request Product Information',
  },
} as const;
